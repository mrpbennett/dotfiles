...
function! s:get_oracle_queries()
  ...
  if !get(g:, 'db_ui_is_oracle_legacy', 0)
...
" Trino: uses --execute flag, TSV output (no headers).
" schemes_tables_query filters out noise schemas to keep initial load fast.
let s:trino_schemes_tables_query = join([
      \ 'SELECT table_schema, table_name FROM information_schema.tables',
      \ "WHERE table_schema NOT IN ('information_schema')",
      \ "AND table_schema NOT LIKE 'db_%'",
      \ "AND table_schema NOT LIKE '%_temp'",
      \ "AND table_schema NOT LIKE 'dp_%'",
      \ "AND table_schema NOT LIKE 'pp_%'",
      \ "AND table_schema NOT LIKE 'temp%'",
      \ "AND table_schema NOT LIKE 'tmp%'",
      \ "AND table_schema NOT LIKE 'test%'",
      \ "AND table_schema NOT LIKE 'user_%'",
      \ ], ' ')

let s:trino = {
      \ 'args': ['--no-progress', '--output-format', 'TSV', '--execute'],
      \ 'schemes_query': 'SHOW SCHEMAS',
      \ 'schemes_tables_query': s:trino_schemes_tables_query,
      \ 'parse_results': {results, min_len -> s:results_parser(results, "\t", min_len)},
      \ 'default_scheme': '',
      \ }
