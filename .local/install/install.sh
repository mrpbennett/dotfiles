...
# Stow first: on a fresh machine there's no ~/.zshrc yet, so oh-my-zsh would
# otherwise write its own template there and stow would later refuse to
# symlink ours over it. Symlinking first + KEEP_ZSHRC=yes below makes
# oh-my-zsh see our .zshrc already in place and leave it alone.
stow --target="$HOME" .
